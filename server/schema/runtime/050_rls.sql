...
------------------------------------------------------------------------------
-- There are no attachment policies, and that is the point
------------------------------------------------------------------------------
--
-- 140_attachments.sql had four, mirroring document_version's. 150 folded the
-- bytes into document_version itself, so the mirror image became the original:
-- a binary revision is admitted by document_version_select, inserted under
-- document_version_insert, and retired the same way a page is.
--
-- wiki.setting has RLS enabled and no policy at all, which is not an omission.
-- RLS with no policy denies every row to every role that is not the owner, so
-- the limit is readable only through
-- wiki.max_attachment_bytes(), which is SECURITY DEFINER. A caller who could
-- read the row could not change it; one who could change it could accept
-- their own uploads.
--
-- wiki.storage_backend is the opposite case and is granted openly. It holds
-- the names of the places bytes can live, which is not a secret and which a
-- client needs in order to know whether it can fetch a body itself.
